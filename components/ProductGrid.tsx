...
//   return (
//     <Card
//       className="border-0 rounded-sm overflow-hidden group flex flex-col bg-white relative"
...
//           alt={product.title}
//           fill
//           className="object-cover transition-all duration-500 ease-in-out group-hover:scale-105 cursor-pointer"
...
//           variant="outline"
//           size={"sm"}
//           className="w-full text-xs text-red-500 border-red-500 hover:bg-red-50 hover:text-red-600 bg-transparent font-semibold cursor-pointer"
//         >
...
function ProductCard({ product }: { product: FormattedProduct }) {
  ...
  const currentImage =
    isHovered && product.image[1] ? product.image[1] : product.image[0];
  const { addToHeart, removeFromHeart, isInHeart } = useHeart();
  const { addToCart, updateQuantity, isInCart, items } = useCart();
  const isLiked = isInHeart(product.id);
  ...
  const handleAddToCart = async () => {
    ...
    // სიმულირებული დაყოვნება ლოუდინგის ვიზუალიზაციისთვის
    ...
    >
      ...
      <div className="px-4 space-y-2 flex-1 flex flex-col justify-between">
        ...
        {!inCart ? (
          ...
          <div className="flex items-center justify-between border border-red-500 rounded-md px-2 py-[5.5px]">
            <button
              onClick={handleDecreaseQuantity}
              className="text-red-500 hover:bg-red-50 px-1 rounded transition-colors"
            >
